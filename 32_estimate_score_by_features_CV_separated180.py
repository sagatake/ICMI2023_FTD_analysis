...
def main():
    ...
    feature_file    = Path('aligned_features_separated180.csv')
    ...
    tgt_phase_indexes = None
    # tgt_phase_indexes = [1] #0-60sec
    # tgt_phase_indexes = [2] #60-120sec
    tgt_phase_indexes = [3] #120-180sec
    ...
    if WITH_FOUR_PAIR:
        ...
        for i in range(1, len(features)):
            ...
            phase_id = features[i][2]
            dream_flag1 = features[i][3]
            dream_flag4 = features[i][4]
            dream_flag7 = features[i][5]
            ...
            if (tgt_phase_indexes != None):
                if not(int(phase_id) in tgt_phase_indexes):
                    continue

            row_features = ['{}_{}'.format(user_id, sess_id)]
            row_features.extend(features[i][6:])
        ...
        features = pick_columns(features, list(range(6, len(features[0]))))
