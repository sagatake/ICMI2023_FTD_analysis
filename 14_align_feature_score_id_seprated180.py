...
def main():
    ...
    feature_file = Path('features_separated180.csv')
    
    dream_flag_file = Path('dataset/data_label/dream_flag.csv')
    
    output_feature_file = Path('aligned_features_separated180.csv')

    output_score_file   = Path('aligned_scores_separated180.csv')
    ...
    feature_header = ['user_id', 'session_id', 'phase_id']
    ...
    for row in sorted(src_feature_list[2:]):
        ...
        try:
            _, user_id, _, session_id, phase_index = row[0].split('_')
            tmp_row = [user_id, session_id, phase_index]
    ...
    print('feature', np.shape(output_feature_list))
    print('score header', np.shape(headers))
    print('score body', np.shape(scores))
    print('score', np.shape(output_score_list))
    
    if OUTPUT:
        if CP932:
            _write_csv(output_feature_file, output_feature_list, 'cp932')
            _write_csv(output_score_file, output_score_list, 'cp932')
        else:
